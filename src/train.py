...
parser.add_argument('--dataset_root', default='/userhome/cs2/wfyleon/a1/ass1_dataset/', type=str, help='dataset root')
...
optimizer = torch.optim.AdamW(hku_mmdetector.parameters(), betas=(0.9, 0.999), lr=args.learning_rate, weight_decay=5e-4)
lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience = 0, factor = 0.85)
# initialize dataset
train_dataset = Dataset(args, split='train', transform=[transforms.ToTensor()])
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4)

###################################################################
# TODO: Please fill the codes below to initialize the exiation dataset
##################################################################
...
for epoch in range(args.num_epochs):
    ...
    Loss_train.append(total_loss.cpu().numpy()/(i+1))
        ##if (i + 1) % 5 == 0:
            ##Visualizer().plot_train_val(loss_train=total_loss / (i + 1))
    ...
    Loss_val.append(validation_loss.cpu().numpy())
    ##Visualizer().plot_train_val(loss_val=validation_loss)
...
plt.ylabel('Loss')
plt.title('Loss and LR vs Epoches')
plt.legend()
